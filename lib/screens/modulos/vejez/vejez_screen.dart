...
class _VejezScreenState extends State<VejezScreen> {
  ...
  Future<void> _analizarYGuardar() async {
    ...
    if (_maltrato) {
      dx = '🚨 Sospecha de maltrato. Activar ruta de protección ICBF / Comisaría de Familia.';
      nivel = 'urgente'; color = Colors.red;
    } else if (presion >= 160) {
      dx = '🚨 Hipertensión severa ($presion mmHg). Remisión urgente. Riesgo de ACV.';
      nivel = 'urgente'; color = Colors.red;
    } else if (spo2 < 92) {
      dx = '😮 SpO2 ${spo2}% — Hipoxemia. Evaluar dificultad respiratoria y remitir urgente.';
      nivel = 'urgente'; color = Colors.red;
    } else if (_caidaReciente) {
      dx = '⚠️ Caída reciente reportada. Evaluar riesgo de fractura y adaptar el hogar.';
      nivel = 'alerta'; color = Colors.orange;
    } else if (!_banoIndep || !_caminaIndep) {
      dx = '🧓 Limitación funcional. Evaluar necesidad de cuidador y terapia ocupacional.';
      nivel = 'alerta'; color = Colors.orange;
    } else {
      dx = '✅ Adulto mayor con funcionalidad conservada. Continuar controles semestrales.';
    ...
    if (_pacienteId != null) {
      ...
      await DatabaseHelper.instance.insertarConsulta({
        'paciente_id': _pacienteId, 'modulo': 'Vejez',
        'fecha': DateTime.now().toIso8601String(),
        'datos_json': jsonEncode({'nombre': _nombreCtrl.text, 'edad': _edadCtrl.text,
          'cuidador': _cuidadorCtrl.text, 'presion': _presionCtrl.text,
          'glucemia': _glucemiaCtrl.text, 'spo2': _spo2Ctrl.text,
          'peso': _pesoCtrl.text, 'fc': _fcCtrl.text,
          'caminaIndep': _caminaIndep, 'comeIndep': _comeIndep, 'banoIndep': _banoIndep,
          'caidaReciente': _caidaReciente, 'medicamentos5mas': _medicamentos5mas,
  ...
  @override
  void dispose() {
    for (final c in [_nombreCtrl, _edadCtrl, _cuidadorCtrl, _presionCtrl, _glucemiaCtrl, _spo2Ctrl, _pesoCtrl, _fcCtrl]) c.dispose();
    super.dispose();
  }
        ...
        title: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Vejez',
              style: const TextStyle(color: Colors.white, fontSize: 17,
                  fontWeight: FontWeight.bold)),
          const Text('60 años o más · Adulto mayor · DISPERSALUD IA',
      ...
      body: SafeArea(
        ...
        child: Column(children: [
          ...
          _Card(titulo: 'Datos del paciente', child: Column(children: [
            ...
            _Campo(label: 'Nombre del cuidador (si aplica)', controller: _cuidadorCtrl),
            const SizedBox(height: 12),
            Row(children: [
            ...
            const SizedBox(height: 12),
            _Campo(label: 'Frecuencia cardíaca (lpm)', controller: _fcCtrl),
          ])),
          const SizedBox(height: 14),
          ...
          _Card(titulo: 'Lista de chequeo', child: Column(children: [
            ...
            _CheckItem(texto: 'Se baña de forma independiente',          activo: _banoIndep,      color: Colors.orange, onChanged: (v) => setState(() => _banoIndep = v)),
            _CheckItem(texto: 'Caída en el último año reportada',        activo: _caidaReciente,  color: Colors.red,    onChanged: (v) => setState(() => _caidaReciente = v)),
            _CheckItem(texto: 'Toma 5 o más medicamentos diarios',       activo: _medicamentos5mas,color: Colors.orange,onChanged: (v) => setState(() => _medicamentos5mas = v)),
            ...
            _CheckItem(texto: 'Neumococo aplicado (>65 años)',           activo: _neumococo,      color: Colors.orange, onChanged: (v) => setState(() => _neumococo = v)),
          ])),
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              ...
              label: Text('Remitir a geriatría',
